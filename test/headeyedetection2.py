...
def main():
    ...
    print('width {0}, height {1}, fps {2}'.format(cam_w, cam_h, fps))
        ...
        for i, pos in enumerate(faces_array):
            ...
            if(OPTIMIZE):
                if(ANG_NOW < 5):
                    ...
                    ANG_AVERAGE = ANG_AVERAGE/5
                    print("ANG AVERAGE IS", ANG_AVERAGE)
                    print("ANG AVERAGE COMPLETE")
                    ANG = True
                    cv2.putText(frame, "Angle Optimize Complete", (10,60),cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,0), 2)
            ...
            for rect in rects:
                ...
                if(OPTIMIZE):
                    if ear < EYE_AR_THRESH:
                        cv2.putText(frame, "Cannot Detect EYE!", cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2)
                    else:
                        if(EYE_NOW < 5):
                            EYE_THRESH_LIST[EYE_NOW]= ear
                            print("EYE_THRESH",EYE_NOW)
                            EYE_AVERAGE = EYE_AVERAGE +ear
                        elif (EYE_NOW == 5):
                            EYE_AVERAGE = EYE_AVERAGE/5
                            print("EYE AVERAGE IS", EYE_AVERAGE)
                            EYE_AR_THRESH = EYE_AVERAGE
                            EYE = True
                        else:
                            break
                        EYE_NOW =EYE_NOW+1
